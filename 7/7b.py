class Verifier():
    ...
    def backtrack(self, curr, index):
        ...
        return self.backtrack(curr + self.args[index], index + 1) or self.backtrack(curr * self.args[index], index + 1) or self.backtrack(
            int(str(curr) + str(self.args[index])), index + 1)
